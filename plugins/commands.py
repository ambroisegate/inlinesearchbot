...
async def start(client, message):
    if message.chat.type in [enums.ChatType.GROUP, enums.ChatType.SUPERGROUP]:
        buttons = [[           
            InlineKeyboardButton('🔔 𝐔𝐩𝐝𝐚𝐭𝐞𝐬', url=f'https://t.me/{SUPPORT_CHAT}')
            ],[
    ...
    if len(message.command) != 2:
        buttons = [[
            InlineKeyboardButton("🤖 𝐀𝐝𝐝 𝐦𝐞 𝐭𝐨 𝐲𝐨𝐮𝐫 𝐠𝐫𝐮𝐩", url=f"http://t.me/{temp.U_NAME}?startgroup=true")
            ],[
            InlineKeyboardButton("🔍 𝐒𝐞𝐚𝐫𝐜𝐡", switch_inline_query_current_chat=''), 
            InlineKeyboardButton("🔔 𝐔𝐩𝐝𝐚𝐭𝐞𝐬", url="https://t.me/hokageclub")
            ],[      
            InlineKeyboardButton("❓ 𝐇𝐞𝐥𝐩", callback_data="help"),
            InlineKeyboardButton("ℹ️ 𝐀𝐛𝐨𝐮𝐭", callback_data="about")
        ]]
    ...
    if len(message.command) == 2 and message.command[1] in ["subscribe", "error", "okay", "help"]:
        buttons = [[
            InlineKeyboardButton("➕ 𝑨𝒅𝒅 𝒎𝒆 𝒕𝒐 𝒚𝒐𝒖𝒓 𝒈𝒓𝒖𝒑", url=f"http://t.me/{temp.U_NAME}?startgroup=true")
            ],[
            InlineKeyboardButton("🔍 𝑺𝒆𝒂𝒓𝒄𝒉", switch_inline_query_current_chat=''), 
            InlineKeyboardButton("🔄 𝑼𝒑𝒅𝒂𝒕𝒆𝒔", url="https://t.me/hokageclub")
            ],[      
            InlineKeyboardButton("❓ 𝑯𝒆𝒍𝒑", callback_data="help"),
            InlineKeyboardButton("ℹ️ 𝑨𝒃𝒐𝒖𝒕", callback_data="about")
        ]]
        reply_markup = InlineKeyboardMarkup(buttons)
        await message.reply_chat_action(enums.ChatAction.TYPING)
        m=await message.reply_sticker("CAACAgUAAxkBAAIFNGJSlfOErbkSeLt9SnOniU-58UUBAAKaAAPIlGQULGXh4VzvJWoeBA")
...
async def settings(client, message):
    ...
    if settings is not None:
        buttons = [[
            ...
            InlineKeyboardButton('🎛️ 𝑏𝑢𝑡𝑡𝑜𝑛 𝑠𝑡𝑦𝑙𝑒', callback_data=f'setgs#button#{settings["button"]}#{str(grp_id)}'),
            InlineKeyboardButton('🔘 𝑠𝑖𝑛𝑔𝑙𝑒' if settings["button"] else '🔲 𝑑𝑜𝑢𝑏𝑙𝑒', callback_data=f'setgs#button#{settings["button"]}#{str(grp_id)}')
            ],[
            InlineKeyboardButton('📩 𝑏𝑜𝑡 𝑝𝑚', callback_data=f'setgs#botpm#{settings["botpm"]}#{str(grp_id)}'),
            InlineKeyboardButton('✅ 𝑦𝑒𝑠' if settings["botpm"] else '🚫 𝑛𝑜', callback_data=f'setgs#botpm#{settings["botpm"]}#{str(grp_id)}')
            ],[                
            InlineKeyboardButton('🔒 𝑓𝑖𝑙𝑒 𝑠𝑒𝑐𝑢𝑟𝑒', callback_data=f'setgs#file_secure#{settings["file_secure"]}#{str(grp_id)}'),
            InlineKeyboardButton('✅ 𝑦𝑒𝑠' if settings["file_secure"] else '🚫 𝑛𝑜', callback_data=f'setgs#file_secure#{settings["file_secure"]}#{str(grp_id)}')
            ],[
            InlineKeyboardButton('🎬 𝑖𝑚𝑑𝑏', callback_data=f'setgs#imdb#{settings["imdb"]}#{str(grp_id)}'),
            InlineKeyboardButton('✅ 𝑦𝑒𝑠' if settings["imdb"] else '🚫 𝑛𝑜', callback_data=f'setgs#imdb#{settings["imdb"]}#{str(grp_id)}')
            ],[
            InlineKeyboardButton('🔤 𝑠𝑝𝑒𝑙𝑙 𝑐ℎ𝑒𝑐𝑘', callback_data=f'setgs#spell_check#{settings["spell_check"]}#{str(grp_id)}'),
            InlineKeyboardButton('✅ 𝑦𝑒𝑠' if settings["spell_check"] else '🚫 𝑛𝑜', callback_data=f'setgs#spell_check#{settings["spell_check"]}#{str(grp_id)}')
            ],[
            InlineKeyboardButton('👋 𝑤𝑒𝑙𝑐𝑜𝑚𝑒', callback_data=f'setgs#welcome#{settings["welcome"]}#{str(grp_id)}'),
            InlineKeyboardButton('✅ 𝑦𝑒𝑠' if settings["welcome"] else '🚫 𝑛𝑜', callback_data=f'setgs#welcome#{settings["welcome"]}#{str(grp_id)}')               
        ]]
        reply_markup = InlineKeyboardMarkup(buttons)
        await message.reply_text(
